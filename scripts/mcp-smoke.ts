...
assert.ok(
  JSON.parse(txt(await client.callTool({ name: "list_rooms", arguments: {} }))).some((r: { id: string }) => r.id === room),
  "list_rooms must include the room just created",
);
console.log("list_rooms includes it -> true");
...
assert.equal(readFileSync(dst, "utf8"), readFileSync(src, "utf8"), "get_file bytes must match the upload");
console.log("get_file round-trip matches -> true");
